...
class Browser:
    ...
    **Constructor**
    ...
    def __init__(self, showWindow=True, proxy=None, downloadPath: str = None, driverPath: str = None, binaryLocation: str = None, arguments=["--disable-dev-shm-usage", "--no-sandbox"]):
        options = webdriver.ChromeOptions()

        if (binaryLocation is not None and binaryLocation != ""):
            binaryPath = os.path.abspath(binaryLocation)
            if (not os.path.isfile(binaryPath)):
                raise FileNotFoundError(
                    errno.ENOENT, os.strerror(errno.ENOENT), binaryPath)
        ...
        if downloadPath is not None and isinstance(downloadPath, str):
            ...
            if (not os.path.isdir(absolutePath)):
                raise FileNotFoundError(
                    errno.ENOENT, os.strerror(errno.ENOENT), absolutePath)
        ...
        if driverPath is not None and isinstance(driverPath, str):
            ...
            if (not os.path.isfile(driverPath)):
                raise FileNotFoundError(
                    errno.ENOENT, os.strerror(errno.ENOENT), driverPath)
        ...
        if driverPath is None:
            ...
            driverPath = os.path.join(os.path.split(
                __file__)[0], 'drivers{0}{1}'.format(os.path.sep, driverfilename))
